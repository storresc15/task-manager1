...
public class PriorityQueue_CategoryGrade implements PriorityDefinitionStrategy {
...
public List<Task> setPriority(TaskOwner theOwner, List<Task> theTasks) throws ParseException {
	...
	private static Task setCombinedPriority(Task theTask, Map<String, Integer> categoryMap) throws ParseException {
		...
		System.out.println("The category searched: " + category);
		System.out.println("The value from the map and category: " + categoryMap.get(category));
		double k = -.9;
		Date d = new Date();
		Date cd = new SimpleDateFormat("yyyy-MM-dd").parse(theTask.getDueDate());		
		
		int CombinedPriority = (int) ( k * ((Math.abs(cd.getTime() - d.getTime()))/600000000) + categoryMap.get(category)/*p*/);
